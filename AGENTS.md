...
cargo run --release --bin kesseldb -- 127.0.0.1:7878 ./data   # single open node
```
...
## Wire protocol (for non-Rust clients)

Length-prefixed `[u32 LE len][payload]`. First payload byte selects mode:
plain `Op::encode()`, `0xFE`+SQL, `0xFD`+session frame (exactly-once),
`0xFC`+token (auth), `0xFB` (stats), `0xFA`+dir (snapshot). Full table in
[`docs/USAGE.md`](docs/USAGE.md) §10.

## Repo map

| Path | What |
|---|---|
| `crates/kesseldb-server` | node binary, engine, single-node + cluster servers |
...
| `crates/kessel-storage` | LSM + WAL + bloom + bounded compaction |
| `crates/kessel-vsr` | Viewstamped Replication + seeded simulator |
| `crates/kessel-{proto,catalog,codec,expr,cache}` | wire types, schema, codec, expr VM, read cache |
| `docs/STATUS.md` | gate table, per-slice status, performance log |
| `docs/USAGE.md` | install, client API, SQL reference, clustering, auth, ops |
