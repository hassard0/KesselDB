...
to prove ORM breadth beyond SQLAlchemy. Honest score: **2/8 smoke
stages**, with ONE clean blocking boundary.

| Django ORM operation | Result | Notes |
|---|---|---|
| `connection.cursor()` + full conn-init | **PASS** (new) | Django's `_configure_timezone` issues `SELECT set_config('TimeZone', $1, false)`; this arc's surgical `set_config(...)` intercept makes connection-init complete |
| `schema_editor.create_model()` (DDL) | **GAP** | Django emits `CREATE TABLE "smokeapp_author" ("id" bigint ... GENERATED BY DEFAULT AS IDENTITY, "name" varchar(32) ...)` — two gaps: **quoted identifiers** + **`GENERATED ... AS IDENTITY`** spelling |
| `Model.objects.create()` (INSERT + RETURNING) | **GAP** | `INSERT INTO "smokeapp_author" ("name") VALUES ($1) RETURNING "smokeapp_author"."id"` — quoted identifiers |
| `Model.objects.values_list()` (SELECT) | **GAP** | `SELECT "t"."id", "t"."name" FROM "t"` — quoted identifiers |
| `.get(pk=…)` / `.filter().update()` / `.filter().delete()` | **GAP** | quoted identifiers (`WHERE "t"."id" = $1`) |

**The boundary is a single SQL-lexer gap: Django UNCONDITIONALLY
double-quotes every identifier** (table, column, qualifier, in DDL / DML
/ projection / WHERE / RETURNING), and kessel-sql's tokenizer rejects
`"` with `unexpected char '"'`. Unlike SQLAlchemy (which emits UNQUOTED
identifiers by default → 7/7 CRUD), Django's quoting is not
configurable, so EVERY Django statement trips the same boundary.

KesselDB's **engine + CRUD semantics are already Django-ready** — fed
the equivalent UNQUOTED, `BIGSERIAL`-spelled SQL, every Django-shaped
operation passes: autoincrement `INSERT … RETURNING id`, qualified
`SELECT t.col FROM t`, and by-PK `UPDATE`/`DELETE WHERE id = $1`. The
gap is purely the SQL *text shape*, not the data path.

**Named follow-up arcs** (priority order): `SP-PG-SQL-QUOTED-IDENT`
(P0 — accept `"ident"` delimited identifiers everywhere; unblocks ~all
Django ORM), `SP-PG-DDL-IDENTITY` (P1 — `GENERATED [BY DEFAULT|ALWAYS]
AS IDENTITY` as a BIGSERIAL alias; Django 6's default PK DDL),
`SP-PG-SQL-AGG-ALIAS-RENDER` (P2 — `COUNT(*) AS "__count"` parse +
FROM-ful aggregate render; Django `.count()`/`.aggregate()`),
`SP-PG-DJANGO-INTROSPECT` (P3 — `connection.introspection` for
`manage.py migrate`/`inspectdb`), `SP-PG-SAVEPOINT` (P3 — nested
`transaction.atomic()` savepoints). Transcript:
