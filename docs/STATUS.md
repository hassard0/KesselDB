...
The full client/server failover path is now done (SP38–42). The remaining
gates are concrete and named: adversarial-partition liveness (seed 7),
auth/quotas/backpressure, ops tooling, and the index point-read perf
enhancement. No vague "research-grade" hedging — each item is a
