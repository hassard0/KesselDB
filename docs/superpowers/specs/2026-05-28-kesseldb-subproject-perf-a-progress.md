...
| **T6** | Eliminate the in-process `Op::encode → apply_raw → Op::decode` roundtrip on the read path (Fix A — `EngineHandle::apply(Op)` bypasses encode+queue+decode for read-only Ops via direct `Arc<RwLock<StateMachine>>` dispatch) + remove `OpResult::Got`'s per-read Vec<u8> alloc on in-process clones (Fix B — variant changes to `Arc<[u8]>`; wire format byte-identical; clones bump refcount). Fix A measured +10.7% lift at N=16 (4.77M → 5.28M ops/sec). Fix B ships the proto-level migration + 3 wire-compat KATs but defers the storage-internal Vec→Arc migration (memtable + SsTable values) to T7 — without that the residual `Storage::get` clone keeps the headline near ~5M ops/sec. Determinism oracle 17/17 green on vulcan against Fix B build. | **DONE_WITH_CONCERNS** | `fb41342` (Fix A) + `25bdb03` (Fix A baseline doc) + `64a5c36` (Fix B variant + callsite migration) + this commit (BENCHMARKS.md §11 + progress tracker + STATUS row) |
| **T7** | Storage-internal Vec→Arc migration: `SsTable::entries: Vec<(Key, Option<Arc<[u8]>>)>` + `Storage::memtable: BTreeMap<Key, Option<Arc<[u8]>>>` + txn-overlay matches + `Storage::get -> Option<Arc<[u8]>>` so the read fast path returns a refcount-bump clone of the on-disk-resident bytes (zero memcpy). Data-row reads dispatch through a new `mvcc::get_at_snapshot_arc` that threads the Arc end-to-end. WAL `Entry` and SSTable on-disk format wire-byte-untouched (replay + SsTable::open wrap-once into Arc). `SnapshotRead::Found(Vec<u8>)` enum shape preserved for off-hot-path callers (Tx::read, SM apply-arm snapshot reads, 100+ tests). Determinism oracle 17/17 green on vulcan against T7 build. Bench sweep landed under heavy cargo contention from a concurrent Track-(stardust) workspace test; absolute numbers carry that caveat. | **DONE_WITH_CONCERNS** | `817ac36` (storage migration + downstream audit + test materialise-Vec) + this commit (BENCHMARKS.md §12 + progress tracker + STATUS row) |
