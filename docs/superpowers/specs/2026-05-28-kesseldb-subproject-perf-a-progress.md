...
| **T7** | Storage-internal Vec→Arc migration: `SsTable::entries: Vec<(Key, Option<Arc<[u8]>>)>` + `Storage::memtable: BTreeMap<Key, Option<Arc<[u8]>>>` + `Storage::get -> Option<Arc<[u8]>>` so the read fast path returns a refcount-bump clone of the on-disk-resident bytes (zero memcpy). Write API (`Storage::put(... Vec<u8>)`) wraps Vec→Arc once at WAL/memtable insert; subsequent reads share. Plus docs + arc closure: STATUS row update + README perf-row update + arc-progress tracker → CLOSED. | **OPEN** | — |
