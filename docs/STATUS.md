...
| **SP92 — corruptible `FaultVfs` + clean-committed-prefix proof** | **partial (honestly scoped)** | New `kessel_io::FaultVfs<V>`: a deterministic, pass-through-by-default disk-fault wrapper (one armed fault — `Torn` half-write or `Err` I/O error — on the *n*-th write to a named file, shared plan via `Rc<RefCell>`); inert until `arm`ed so every existing test is unaffected. Proven: `wal_torn_write_recovers_clean_committed_prefix` — a torn WAL write leaves a **clean committed prefix** (`Storage::open` recovers every op before the tear and *nothing* at/after it — no partial/garbage op), deterministically. This is the exact invariant VSR safety rests on. **Honest boundary:** the *multi-node* disk-fault-*during-view-change* harness is **still tracked, not delivered** — it needs SM-reopen→VSR-rejoin plumbing the cluster sim lacks today (crash is modelled as `crashed=true`, never a disk reopen; `StateMachine::apply` has no op-number replay guard, so a reopened-then-state-transferred node would double-apply). `FaultVfs` is the reusable primitive that unblocks it. **198 green**, seed-7 intact |
...
injected *precisely during* a view change — the corruptible-Vfs
primitive now exists (SP92 `kessel_io::FaultVfs`, with the
clean-committed-prefix invariant proven at the storage layer), but
the *multi-node* during-view-change harness is still open: it needs
SM-reopen→VSR-rejoin plumbing the cluster sim lacks (crash is
`crashed=true`, never a disk reopen; `apply` has no op-number replay
guard). Scoped follow-up, not faked; storage torn-write/crash
recovery and partition/heal are already tested. Cluster membership
...
Still out of scope (each a later spec): the `MIN`/`MAX` aggregate
fast-path on string `RANGE INDEX` columns (string `RANGE INDEX` +
`FindRange` + SQL-planner range-narrowing are delivered — SP87/SP90;
only the SP73-style `bound_in` aggregate shortcut stays numeric-only,
string `MIN`/`MAX` still correct via the verified scan; the same
shortcut on `U128`/`I128` columns is likewise still scan-correct
only), cross-shard scatter-gather *reads* / SQL-text routing (distinct from
