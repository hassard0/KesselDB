...
Still out of scope (each a later spec): string range indexes wired
into the SQL-planner range-narrowing & the `MIN`/`MAX` fast-path
(string `RANGE INDEX` + `FindRange` are delivered; planner/agg
acceleration on string columns is numeric-only — string is still
correct via the verified scan), `U128`/`I128` ordered indexes,
