...
(***************************************************************************)
(* CommitTxWritesVersionedKeyspaceOnly — NARROWED form.                   *)
(*                                                                          *)
(* The original Decision 9 LegacyKeyspaceEmpty invariant cannot be        *)
(* asserted because the 14 data-row apply arms still write the legacy    *)
(* keyspace (deferred to SP116). What we CAN assert is that ops that go  *)
(* through the Op::CommitTx soft-accept path land in the versioned       *)
(* keyspace ONLY — the SI/SSI write-side is the canonical entry point.   *)
(*                                                                          *)
(* In TLA+ terms: every commit modeled by CommitTxSoftAccept (= CommitGc *)
(* with the effective_commit_opnum) installs versions into the abstract  *)
(* `versions` state variable. No CommitTxSoftAccept writes outside       *)
(* `versions`. (The 14 data-row apply arms are NOT modeled by the        *)
(* `versions` map; they are out of scope.)                                 *)
(*                                                                          *)
(* The mechanical form: for every Committed Tx t (status = Committed),    *)
(* every write_set entry installed by CommitTxSoftAccept appears in       *)
(* `versions` at opnum = commit_opnum AND no other state is mutated by   *)
(* a CommitTxSoftAccept action (the action delegates entirely to          *)
(* CommitGc which only touches versions + txs + txsSi + pendingTxs +      *)
(* rwEdges + opCount).                                                     *)
(*                                                                          *)
(* This is essentially the CommitAtomicityGc invariant from MVCCGc        *)
(* applied to the soft-accept path. We restate it here at the cutover    *)
(* layer for documentation; it's already carried forward via EXTENDS.    *)
(* The fresh content of this NARROWED invariant is the EXPLICIT marking  *)
(* that the cutover infrastructure ONLY exercises the versioned          *)
(* keyspace; the legacy keyspace remains as-is for the 14 deferred arms. *)
(*                                                                          *)
(* Mechanical assertion: for every Committed Tx t with write_set                                                                          *)
(* non-empty and commit_opnum >= lowWaterMark, every (k, v) in            *)
(* write_set IS present in versions[k] at opnum = commit_opnum. This IS  *)
(* CommitAtomicityGc; we assert it again here under a new name to make   *)
(* the "narrowed cutover SI/SSI write-side ONLY" claim inspectable.      *)
(***************************************************************************)
CommitTxWritesVersionedKeyspaceOnly ==
    \A t \in TxIds :
