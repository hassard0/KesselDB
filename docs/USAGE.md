...
- **`SP-PG-DDL-IDENTITY`** (P1) — Django 6's default `BigAutoField`
  renders `… GENERATED BY DEFAULT AS IDENTITY`, not `BIGSERIAL`; the
  create_model DDL still needs this spelling accepted as a
  BIGSERIAL-autoincrement alias.
- **`SP-PG-SQL-AGG-ALIAS-RENDER`** (P2) — `SELECT COUNT(*) AS "__count"
  FROM t` (Django `.count()`/`.aggregate()`): parse `COUNT(*) AS
  <alias>` + PG-wire render of a FROM-ful aggregate projection. (Probed
  directly: the quoted `DELETE` executes; only the trailing count trips.)
